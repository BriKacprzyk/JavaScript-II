...
const fullNames = runners.forEach((names) => {
...
const firstNamesAllCaps = runners.map((names) => {
  return{'first_name': names.first_name.toUpperCase()};
});
...
const runnersLargeSizeShirt = runners.filter((shirt) => {
  return shirt.shirt_size = 'L';
});
console.log(runnersLargeSizeShirt);

// ==== Challenge 4: Use .reduce() ====
// The "donation"s need to be tallied up and reported for tax purposes. Add up all the "donation"s and save the total into a ticketPriceTotal variable.
...
// Problem 1
//A problem has occured where people that donated over 150 get a small trophy find who gets a trophy.

const donationPrize = runners.filter((money) => {
  return money.donation >= 150;
});
console.log(donationPrize);

// Problem 2
//The marathon has decided to customize the runners shirts to say their last name and id number. find those items.
const custom = runners.forEach((names) => {
  console.log(names.last_name, names.id);
});
console.log (custom);

// Problem 3
//contestants with the id of 40+ are winning a candy basket. Who are they?
const candyBasket = runners.filter((num) => {
  return num.id >= 40;
});
console.log(candyBasket);